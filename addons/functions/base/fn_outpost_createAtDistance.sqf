...
private _garrison = [ A3A_faction_reb get "unitRifle"];
...
waitUntil {
	...
	or ({alive _x} count units _groupX == 0)
  or (!(_markerX in atpostsFIA))
};
...
waitUntil {sleep 1; (spawner getVariable _markerX == DESPAWNPLAYER) or (!(_markerX in atpostsFIA))};
