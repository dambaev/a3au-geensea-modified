...
if (player == leader _unit) then {
    ...
    _is_player = isPlayer _unit;
		if (!_is_player && unitReady _unit) then {
			if ((alive _unit) and (_unit distance (getMarkerPos respawnTeamPlayer) > 50) and (_unit distance leader group _unit > 500) and ((vehicle _unit == _unit) or ((typeOf (vehicle _unit)) in arrayCivVeh))) then {
				...
				waitUntil {sleep 1;(!alive _unit) or (_unit distance player < 500) or (time > _timeX) or (isPlayer _unit)};
        _is_player = isPlayer _unit;
				if ((_unit distance player >= 500) and (alive _unit) and (!_is_player)) then {_unit setPos (getMarkerPos respawnTeamPlayer)};
        if( !_is_player) then {
				  [_unit] join group player;
        };
			};
		} else {
      if( _is_player && (!isNil "A3A_fnc_FIAinit_debug")) then {
        Info_2("wanted to move unit %1 of the group %2 to group leader, but this is a player", name _unit);
      };
    };
	};
} else {
